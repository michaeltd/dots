#!/usr/bin/env bash
#
# https://linoxide.com/linux-shell-script/linux-memory-usage-program/
...
convert() {
  ...
  while [ $(echo "${value} > 1024"|bc) -eq 1 ]; do
...
# process(2) in output
for Name in `cat /tmp/res2 | awk '{print $6}' | sort | uniq`; do
...
while read line; do
  echo $line | while read a b c d e f; do
