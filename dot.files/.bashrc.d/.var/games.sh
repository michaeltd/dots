...
rps() {
  # Rock Paper Scissors mt 20170525
    declare -ra op=( "${red}Rock${reset}" "${green}Paper${reset}" "${blue}Scissors${reset}" )
    declare -ra oc=( "${green}WIN${reset}" "${red}Defeat${reset}" "${yellow}Draw${reset}" )
    declare -rA rs=( [0,0]=${oc[2]} [0,1]=${oc[1]} [0,2]=${oc[0]} \
		     [1,0]=${oc[0]} [1,1]=${oc[2]} [1,2]=${oc[1]} \
		     [2,0]=${oc[1]} [2,1]=${oc[0]} [2,2]=${oc[2]} )
